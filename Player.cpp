...
void Player::onCollision(ColidableObject* object)
{
	...
	if (Ball* ball = dynamic_cast<Ball*>(object))
	{
		...
		this->getPlayerStats()->set_current_health(this->getPlayerStats()->get_current_health() - ball->getStatistics().get_damage());
	...
	else if (Trap* trap = dynamic_cast<Trap*>(object))
	{
		...
		case 6:
			...
			break;
		case 9:
			if (!hasElement(current_status, 9)) current_status.push_back(StatusStringHud(9, "Slow", 5));
