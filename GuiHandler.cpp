...
void GuiHandler::statistics()
{
	...
	//Set values
	skillpoints_request << 4 << std::string(current_picked_character);//current_picked_character;
	...
	skills_request << 2 << std::string(current_picked_character);//current_picked_character;
	...
	if (tree.getStatus(1) == 2) {
		...
		buttonFK1->setSize(24, 24);
		buttonFK1->setPosition(538, 266);
		//buttonFK1->connect("clicked", []() {std::cout << "-->FK1Pressed() " << std::endl; });
	...
	if (tree.getStatus(2) == 2) {
		...
		buttonFV1->setSize(24, 24);
		buttonFV1->setPosition(570, 266);
		//buttonFV1->connect("clicked", []() {std::cout << "-->FV1Pressed() " << std::endl; });
		buttonFV1->connect("clicked", &GuiHandler::buySkill, this, 1, "Valium", &tree);
		gui->add(buttonFV1);
	}
	//
	if (tree.getStatus(3) == 2) {
		...
		buttonFW1->setSize(24, 24);
		buttonFW1->setPosition(554, 250);
		//buttonFW1->connect("clicked", []() {std::cout << "-->FW1Pressed() " << std::endl; });
...
{
	sf::Packet request_packet = tree->buyRequest(character, idS);
